                            ...
                            <table class="table" id="storestable">
                                ...
                                <tbody>
                                    @foreach ($services as $key => $service)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td><img src="{{ asset('uploads/' . $service->image) }}" width="80" height="80" alt=""></td>
                                            <td>{{ $service->title }}</td>
                                            <td>{{ $service->price }}</td>
                                            <td>{{ $service->category->name }}</td>
                                            <td>

                                                <input type="checkbox" data-id="{{ $service->id }}" name="status" class="js-switch allssee"
                                                {{ $service->status == 1 ? 'checked' : '' }}>

                                            </td>
                                            <td>
                                                <a href="{{ route('services.edit', $service->id) }}" class="btn btn-warning">{{ __('تعديل') }}</a>
